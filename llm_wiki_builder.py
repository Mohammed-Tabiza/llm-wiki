...
Return JSON only. Use this JSON schema as the contract:
{output_schema}
...
class KnowledgeItem(BaseModel):
    name: str = Field(description="Canonical topic or entity name.")
    summary: str = Field(description="Short source-grounded summary.")


class SourceAnalysisResult(BaseModel):
    summary: str = Field(description="Two to three dense source-grounded sentences.")
    key_points: list[str] = Field(description="Three to five atomic, non-overlapping facts.")
    topics: list[KnowledgeItem] = Field(description="Up to three broad reusable concepts.")
...
def invoke_json(template: str, **kwargs: Any) -> dict[str, Any]:
    ...
    kwargs = {**kwargs, "output_schema": SourceAnalysisResult.model_json_schema()}
